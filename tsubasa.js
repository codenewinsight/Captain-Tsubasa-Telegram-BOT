...
class Tsubasa {
    ...
    async callDailyRewardAPI(initData, axiosInstance) {
        ...
        try {
            ...
            if (dailyRewardResponse.status === 200) {
                ...
                return { success: false, message: "Daily Check-in Claimed before" };
            }
        } catch (error) {
    ...
    async levelUpCards(initData, totalCoins, axiosInstance) {
        ...
        do {
            ...
            if (!cardInfo) {
                console.log("Get Card infor failed. Skip Upgrade!");
                break;
            }
            ...
            for (const card of sortedCards) {
                ...
                if (card.end_datetime && currentTime > card.end_datetime) {
                    this.log(`Card ${card.name} (${card.cardId}) Expried. Skip Upgrade.`, 'warning');
                    ...
                    try {
                        ...
                        if (error.response && error.response.status === 400) {
                            if (error.response.data && error.response.data.message === 'Wait for cooldown') {
                                this.log(`Cooldown time for card ${card.name} (${card.cardId})`, 'warning');
                                cooldownCards.add(card.cardId);
                            } else if (error.response.data && error.response.data.message === 'Expired') {
                                this.log(`Error Card Upgrade ${card.name} (${card.cardId}): Card Expried`, 'warning');
                                errorCards.add(card.cardId);
                            } else {
    ...
    async callEnergyRecoveryAPI(initData, axiosInstance) {
        ...
        try {
            ...
            if (recoveryResponse.status === 200) {
                ...
                return { success: false, error: `Can't re-fill Energy` };
            }
        } catch (error) {
            return { success: false, error: `Can't re-fill Energy` };
        }
        ...
        while (continueProcess) {
            ...
            while (currentEnergy > 0) {
                ...
                if (recoveryResult.energy === maxEnergy) {
                    ...
                    this.log(`Re-fill Energy Success | Energy Balance: ${currentEnergy}/${maxEnergy}`, 'success');
                } else {
                    this.log(`Insufficient energy re-fill | Energy Balance: ${recoveryResult.energy}/${maxEnergy}`, 'warning');
                    continueProcess = false;
                    break;
                }
            }
    ...
    async callTapLevelUpAPI(initData, axiosInstance) {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                return { success: false, error: `Upgrade tap failed: ${response.status}` };
            }
        } catch (error) {
            return { success: false, error: `Upgrade tap failed: ${error.message}` };
        }
    }
    ...
    async callEnergyLevelUpAPI(initData, axiosInstance) {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                return { success: false, error: `Upgrade energy failed: ${response.status}` };
            }
        } catch (error) {
            return { success: false, error: `Upgrade energy failed: ${error.message}` };
        }
    }
